          ...
          <form onSubmit={handleSubmit} className="p-7 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                onClick={async () => {
                  ...
                  if (error) {
                    ...
                    if (error.message?.includes('not enabled') || error.message?.includes('validation_failed')) {
                      setError('Google OAuth provider is not yet enabled in your Supabase Dashboard. Go to Supabase > Authentication > Providers > Google, paste your Client ID & Secret, and set redirect URL to ' + window.location.origin);
                    } else {
                      setError(error.message);
                    }
                  }
                }}
                className="py-3 px-3 rounded-2xl bg-white hover:bg-stone-50 border-2 border-stone-200 hover:border-amber-300 text-stone-700 font-bold text-xs flex items-center justify-center space-x-2 shadow-2xs transition active:scale-[0.98] disabled:opacity-60"
              >
                {oauthLoading === 'google' ? (
              ...
              >
                ...
                <span>{oauthLoading === 'azure' ? 'Redirecting…' : 'Microsoft'}</span>
              </button>
            </div>
